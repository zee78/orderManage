  ...
  <div class="col-lg-12">
    <div class="card">
      <div class="card-body">
        ...
        <form id="update-skill-form" enctype="multipart/form-data">
          @csrf
          <div class="form-group row mb-4">
            <label for="projectname" class="col-form-label col-lg-2">Skill Name</label>
            <div class="col-lg-10">
              <input id="skillname" name="skill_name" value="{{$getSingleData->skill_name}}" type="text" class="form-control" placeholder="Enter Skill Name...">
            </div>
          </div>
        </form>
        <div class="row justify-content-end">
          <div class="col-lg-10">
            <button type="submit" form="update-skill-form" class="btn btn-primary">Edit Skill</button>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  $('#update-skill-form').on('submit', function(event){
      event.preventDefault();
      let createFormDatas = $('#update-skill-form').serialize();
    $.ajax({
        url: "{{url('admin/skills/'.$getSingleData->id)}}",
        type: 'PATCH',
        data: createFormDatas,
        // contentType: false,
        processData: false,
